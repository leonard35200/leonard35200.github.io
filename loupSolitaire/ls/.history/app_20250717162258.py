...
function wrapSelectedText() {
  ...
  if (!parentP) {
    ...
    try {
      ...
      alert("Impossible d'entourer la sélection avec un paragraphe. Sélection trop complexe.");
      return;
    }
  }

  // Créer l'élément <strong>
  const strong = document.createElement("strong");
  strong.textContent = selection.toString();

  // Remplacer la sélection par <strong>
  range.deleteContents();
  range.insertNode(strong);

  // Prompt pour les attributs data-*
  ...
  const val4 = prompt("Valeur pour data-bonus (ex : 0 hab) ?", "0hab");
