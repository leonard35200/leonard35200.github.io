...
class NavigationManager {
  ...
  const lancerCombat = (index) => {
    if (index >= combats.length) {
      ...
      p.appendChild(msg);
      // Ici tu peux débloquer la navigation ou afficher la suite
      return;
    }
    const ennemi = combats[index];
    this.lancerCombatUnique(ennemi, () => {
      // Quand ce monstre est mort, passe au suivant
...
// Ajoute cette méthode dans ta classe NavigationManager
lancerCombatUnique(ennemi, onWin, p) {
  // Supprime une éventuelle ancienne fenêtre de combat
  const old = p.querySelector('.combat-popup');
  if (old) old.remove();
  ...
  div.innerHTML = `
    <div style="background:#222; color:#fff; border:2px solid #c00; padding:1em; margin-top:1em; text-align:center; border-radius:12px; box-shadow:0 4px 16px #000a;">
      <div style="font-size:1.5em; margin-bottom:1em;">COMBAT contre <b>${ennemi.nom}</b></div>
      <div>Habileté : <b>${ennemi.hab}</b> | Endurance : <b id="vieMonstre">${ennemi.end}</b></div>
      <button id="btnMonstreMoins2" style="margin:1em;">-2 Endurance Monstre</button>
      <br>
      <button id="btnVaincu" style="margin-top:1em;background:#0a0;">Monstre vaincu</button>
    </div>
  `;
  p.appendChild(div);

  // Gestion du bouton pour diminuer l'endurance du monstre
  let vie = ennemi.end;
  div.querySelector('#btnMonstreMoins2').onclick = () => {
    vie = Math.max(0, vie - 2);
    div.querySelector('#vieMonstre').textContent = vie;
  };

  // Quand le monstre est vaincu, on passe au suivant
  div.querySelector('#btnVaincu').onclick = () => {
    div.remove();
    onWin();
  };
}
