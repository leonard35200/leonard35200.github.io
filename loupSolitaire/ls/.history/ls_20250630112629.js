...
class NavigationManager {
  ...
  if (!p || !p.innerHTML.includes('<br><br><strong>')) return;

  console.log(`[Combat] Détection du paragraphe ${id}`);

  // Suppression de l’ancienne popup
  const ancien = p.querySelector('.combat-popup');
  if (ancien) {
    console.log("[Combat] Ancienne popup trouvée, suppression.");
    ancien.remove();
  }

  // Récupération des stats du monstre
  let vieMonstre = 10, habMonstre = 10;
  const mEnd = p.innerHTML.match(/ENDURANCE\s*[:|：]?\s*(\d+)/i);
  const mHab = p.innerHTML.match(/HABILET[ÉE]\s*[:|：]?\s*(\d+)/i);
  if (mEnd)   vieMonstre = +mEnd[1];
  if (mHab)   habMonstre = +mHab[1];
  localStorage.setItem('stat_monstre', vieMonstre);
  console.log(`[Combat] Monstre -> END=${vieMonstre}, HAB=${habMonstre}`);

  // Stats du héros
  let habHero = +localStorage.getItem('stat_hab');
  if (isNaN(habHero)) {
    const ih = document.getElementById('hab');
    habHero = ih ? +ih.value : 15;
  }
  console.log(`[Combat] Héros -> HAB=${habHero}`);

  // Bonus psy et arme
  const armesPsy      = Object.values(localStorage).some(v => v?.includes("Puissance psychique"));
  const bouclierPsy   = Object.values(localStorage).some(v => v?.includes("Bouclier psychique"));
  let bonusArme = 0;
  const armeMaitrisee = localStorage.getItem('arme_maitrisee');
  const disp1         = localStorage.getItem('discipline1') || "";
  if (disp1.includes("Maîtrise des armes") &&
      [localStorage.getItem('arme1'), localStorage.getItem('arme2')].includes(armeMaitrisee)) {
    bonusArme = 2;
  }
  console.log(`[Combat] armesPsy=${armesPsy} bouclierPsy=${bouclierPsy} bonusArme=${bonusArme}`);

  // Création du container
  const div = document.createElement('div');
  div.className = 'combat-popup';
  console.log("[Combat] Création de la div.combat-popup");

  // Déclaration de la fonction avant les listeners
  function afficherBarres(bPsy, msgPsy) {
    console.log(`[Combat] afficherBarres appelé avec bonusPsy=${bPsy}`);
    const quotient   = habHero + bonusArme + bPsy - habMonstre;
    console.log(`[Combat] quotient calculé = ${quotient}`);
    const vieHero    = +localStorage.getItem('stat_end')     || 35;
    const vieHeroMax = +localStorage.getItem('stat_end_max') || vieHero;
    ...
    `;
    const zone = div.querySelector('#zoneBarresCombat');
    zone.innerHTML = html;
    zone.style.display = '';
    console.log("[Combat] Barres injectées en HTML");

    // Mise en place du style si nécessaire (identique à avant)
    if (!document.getElementById('style-barres-combat')) {
      const style = document.createElement('style');
      style.id = 'style-barres-combat';
      style.textContent = `/* ... vos règles CSS ... */`;
      document.head.appendChild(style);
      console.log("[Combat] Style de barres ajouté au <head>");
    }

    // Initialisation des valeurs
    let vh = vieHero, vm = vieMonstre;
    function maj(idBarre, idRest, idIco, val, max, icoP, icoV) {
      const b = div.querySelector('#'+idBarre);
      const r = div.querySelector('#'+idRest);
      const i = div.querySelector('#'+idIco);
      if (!b||!r||!i) return;
      b.style.width = (val/max*100)+'%';
      r.textContent = val;
      i.textContent = val===0?icoV:icoP;
    }
    maj('vieHeroBarre','vieHeroRestante','iconeHeroVie',vh,vieHeroMax,'❤️','💀');
    maj('vieMonstreBarre','vieMonstreRestante','iconeMonstreVie',vm, vieMonstre,'👹','💀');
    console.log("[Combat] Barres mises à jour initialement");

    // Listeners des boutons de dégâts
    div.querySelector('#btnHeroMoins2').onclick = () => {
      vh = Math.max(0, vh - 2);
      localStorage.setItem('stat_end', vh);
      document.getElementById('end').value = vh;
      maj('vieHeroBarre','vieHeroRestante','iconeHeroVie',vh,vieHeroMax,'❤️','💀');
      console.log(`[Combat] -2 Héros appliqué, nouvelle vie=${vh}`);
    };
    div.querySelector('#btnMonstreMoins2').onclick = () => {
      vm = Math.max(0, vm - 2);
      localStorage.setItem('stat_monstre', vm);
      maj('vieMonstreBarre','vieMonstreRestante','iconeMonstreVie',vm, vieMonstre,'👹','💀');
      console.log(`[Combat] -2 Monstre appliqué, nouvelle vie=${vm}`);
    };
  }

  // Construction du HTML initial
  let html = `
  ...
  if (armesPsy) {
    html += `
      <div id="confirmationPsy" style="margin-bottom:1em">
        <b>Discipline psychique détectée :</b><br>
        <button id="btnPsy2" style="margin:.5em">Activer bonus +2</button>
        <button id="btnPsy0" style="margin:.5em">Aucun bonus</button>
      </div>
    `;
  }
  html += `<div id="zoneBarresCombat" style="display:none"></div></div>`;

  div.innerHTML = html;
  p.appendChild(div);
  console.log("[Combat] Popup insérée dans le paragraphe");

  // Binding des boutons psy
  if (armesPsy) {
    console.log("[Combat] Liaison des boutons psy");
    const btn2 = div.querySelector('#btnPsy2');
    const btn0 = div.querySelector('#btnPsy0');

    if (btn2) {
      btn2.addEventListener('click', () => {
        console.log("[Combat] btnPsy2 cliqué");
        div.querySelector('#confirmationPsy')?.remove();
        console.log("[Combat] confirmationPsy supprimé");
    ...
    if (btn0) {
      btn0.addEventListener('click', () => {
        console.log("[Combat] btnPsy0 cliqué");
        div.querySelector('#confirmationPsy')?.remove();
        console.log("[Combat] confirmationPsy supprimé");
        div.querySelector('#zoneBarresCombat').style.display = '';
        afficherBarres(0, '');
    ...
    console.log("[Combat] Pas de bonus psy, j'affiche directement");
    afficherBarres(0, '');
  }
}
