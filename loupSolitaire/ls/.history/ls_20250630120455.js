...
class NavigationManager {
  ...
  const anciennePopup = p.querySelector('.combat-popup');
  if (anciennePopup) anciennePopup.remove();

  // Récupère les stats du monstre
  let vieMonstre = parseInt((p.innerHTML.match(/ENDURANCE\s*[:|：]?\s*(\d+)/i) || [])[1]) || 10;
  let habMonstre = parseInt((p.innerHTML.match(/HABILET[ÉE]\s*[:|：]?\s*(\d+)/i) || [])[1]) || 10;
  ...
  const aDisciplineArme = localStorage.getItem('discipline1') === `Maîtrise des armes (+2 hab. si possède ${armeMaitrisee})`;
  const possedeArme = [localStorage.getItem('arme1'), localStorage.getItem('arme2')].includes(armeMaitrisee);
  if (aDisciplineArme && possedeArme) bonusArme = 2;
  ...
  // Si pouvoir psy, on affiche les options
  if (armesPsy || bouclierPsy) {
    html += `<div id="confirmationPsy" style="margin-bottom:1em;">
      <b>Discipline psychique détectée :</b><br>`;
    if (armesPsy && bouclierPsy) {
      html += `<button id="btnPsy4" style="margin:0.5em;">+4 (Arme + Bouclier)</button>`;
    }
    html += `<button id="btnPsy2" style="margin:0.5em;">+2 (Psychique)</button>
  ...
  // Affiche les barres
  const afficherBarres = (bonusPsy, messagePsy = "") => {
    ...
    const zone = popup.querySelector('#zoneBarresCombat');
    zone.innerHTML = `
      <div class="barre-container"><div class="vie-remplissage" id="vieHeroBarre"></div>
        <div class="contenu-barre"><div class="coeur" id="iconeHeroVie">❤️</div>
        <div class="nom">Héros</div><div class="rond-vie" id="vieHeroRestante">${vieHero}</div></div>
      </div>
      <button id="btnHeroMoins2">-2 Héros</button>
      <div class="barre-container"><div class="vie-remplissage" id="vieMonstreBarre"></div>
        <div class="contenu-barre"><div class="coeur" id="iconeMonstreVie">👹</div>
        <div class="nom">Monstre</div><div class="rond-vie" id="vieMonstreRestante">${vieMonstre}</div></div>
      </div>
    ...
    const majBarre = (idBarre, idRond, idIcone, vie, vieMax, iconePleine, iconeVide) => {
      const barre = popup.querySelector("#" + idBarre);
      const texte = popup.querySelector("#" + idRond);
      const icone = popup.querySelector("#" + idIcone);
      if (barre && texte && icone) {
        barre.style.width = (vie / vieMax * 100) + "%";
        texte.textContent = vie;
        icone.textContent = vie > 0 ? iconePleine : iconeVide;
      }
    };
    ...
    majBarre("vieHeroBarre", "vieHeroRestante", "iconeHeroVie", vieH, vieHeroMax, "❤️", "💀");
    majBarre("vieMonstreBarre", "vieMonstreRestante", "iconeMonstreVie", vieM, vieMonstreMax, "👹", "💀");

    popup.querySelector("#btnHeroMoins2").onclick = () => {
      vieH = Math.max(0, vieH - 2);
      majBarre("vieHeroBarre", "vieHeroRestante", "iconeHeroVie", vieH, vieHeroMax, "❤️", "💀");
      localStorage.setItem('stat_end', vieH);
      const inputEnd = document.getElementById('end');
      if (inputEnd) inputEnd.value = vieH;
    };

    popup.querySelector("#btnMonstreMoins2").onclick = () => {
      vieM = Math.max(0, vieM - 2);
      majBarre("vieMonstreBarre", "vieMonstreRestante", "iconeMonstreVie", vieM, vieMonstreMax, "👹", "💀");
      localStorage.setItem('stat_monstre', vieM);
    };
  };

  // Gestion des boutons psychiques
  setTimeout(() => {
    if (armesPsy && bouclierPsy) {
      popup.querySelector("#btnPsy4")?.addEventListener("click", () => {
        popup.querySelector("#confirmationPsy")?.remove();
        afficherBarres(4, "(Arme + Bouclier Psychique)");
      });
    }
    if (armesPsy || bouclierPsy) {
      popup.querySelector("#btnPsy2")?.addEventListener("click", () => {
        popup.querySelector("#confirmationPsy")?.remove();
        afficherBarres(2, "(Psychique)");
      });
      popup.querySelector("#btnPsy0")?.addEventListener("click", () => {
        popup.querySelector("#confirmationPsy")?.remove();
        afficherBarres(0, "");
      });
    } else {
      // Si pas de psy, on affiche directement
      afficherBarres(0, "");
    }
  }, 20);
}
