...
class NavigationManager {
  ...
  // ======================
// Détection et lancement du combat (sans objets)
// ======================
detecterCombatDansParagraphe(id) {
  const p = document.getElementById(id);
  if (!p) return;
  p.querySelectorAll('.combat-popup').forEach(el => el.remove());

  // stats monstre
  const matchEnd = p.innerHTML.match(/ENDURANCE\s*[:|：]?\s*(\d+)/i);
  const matchHab = p.innerHTML.match(/HABILET[ÉE]\s*[:|：]?\s*(\d+)/i);
  const vieMonstreInit = matchEnd ? parseInt(matchEnd[1],10) : 10;
  localStorage.setItem('stat_monstre_max', vieMonstreInit);
  localStorage.setItem('stat_monstre', vieMonstreInit);

  // stats héros
  const vieHeroInit = parseInt(localStorage.getItem('stat_end'),10) || 20;
  localStorage.setItem('stat_end_max', vieHeroInit);

  // affichage popup toujours visible
  const div = document.createElement('div');
  div.className = 'combat-popup';
  div.innerHTML = `
    <div class="popup-header">COMBAT</div>
    <div id="zoneBarresCombat"></div>
    <button id="btnStart">Commencer le combat</button>
  `;
  p.appendChild(div);

  div.querySelector('#btnStart').addEventListener('click', ()=>{
    afficherBarres( /* bonusPsy */ 0, /* msgPsy */ '');
  });
}
...
// ======================
function afficherBarres(bonusPsy, msgPsy) {
  ...
  const vieMonMax  = parseInt(localStorage.getItem('stat_monstre_max'), 10) || 10;
  const vieMon     = parseInt(localStorage.getItem('stat_monstre'),     10) || vieMonMax;

  const zone = document.querySelector('.combat-popup #zoneBarresCombat');
  if (!zone) return;

  zone.innerHTML = `
    <div class="barre" id="hero">
      <div class="remplissage"></div>
      <span>${vieHero}/${vieHeroMax}</span>
    </div>
    <div class="barre" id="monstre">
      <div class="remplissage"></div>
      <span>${vieMon}/${vieMonMax}</span>
    </div>
    <button id="hitHero">-2 Héros</button>
    <button id="hitMonstre">-2 Monstre</button>
  `;

  function maj(id, cur, max) {
    const bar = zone.querySelector(`#${id} .remplissage`);
    if (bar) bar.style.width = (cur / max * 100) + '%';
  }
  maj('hero', vieHero, vieHeroMax);
  maj('monstre', vieMon, vieMonMax);

  zone.querySelector('#hitHero').onclick = () => {
    const nv = Math.max(0, vieHero - 2);
    localStorage.setItem('stat_end', nv);
    document.getElementById('end').value = nv;
    maj('hero', nv, vieHeroMax);
  };
  zone.querySelector('#hitMonstre').onclick = () => {
    const nv = Math.max(0, vieMon - 2);
    localStorage.setItem('stat_monstre', nv);
    maj('monstre', nv, vieMonMax);
  };
}
