...
if (!tutoVu && tutoScreen && btnNext && btnPrev && tutoPages.length > 0) {
  ...
  let window._tutoPage = 0;

  const updateTuto = () => {
const box = document.querySelector('#tuto-screen .intro-content');
if (box) box.scrollTop = 0;
    tutoPages.forEach((page, i) => {
      page.classList.toggle('hidden', i !== window._tutoPage);
    });
    btnPrev.disabled = window._tutoPage === 0;
    btnNext.textContent = window._tutoPage === tutoPages.length - 1 ? "Commencer l’aventure ▶️" : "Suivant ▶️";
if (window._tutoPage === 4) {
  ...
  btnNext.addEventListener('click', () => {
    if (window._tutoPage < tutoPages.length - 1) {
      window._tutoPage++;
      updateTuto();
    } else {
...
["discipline1","discipline2","discipline3","discipline4","discipline5"].forEach(id => {
  ...
  if (el) {
    el.addEventListener("change", () => {
      if (window._tutoPage === 4) updateTuto();
    });
  }
});
