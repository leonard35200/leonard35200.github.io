...
class NavigationManager {
  ...
  detecterCombatDansParagraphe(id) {
    ...
    if (p.innerHTML.includes('<br><br><strong>')) {
      console.log("⚔️ Combat détecté dans ce paragraphe !");
    }
  }
...
window.gotoParagraph = showParagraph;
window.currentParagraphId = localStorage.getItem('currentParagraph') || 'paraIntro';
Object.defineProperty(window, 'currentParagraph', {
  get() {
    return window.currentParagraphId;
  },
  set(val) {
    window.currentParagraphId = val;
    showParagraph(val);
  }
});
