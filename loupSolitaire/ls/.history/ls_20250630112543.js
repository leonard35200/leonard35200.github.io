...
class NavigationManager {
  ...
  function afficherBarres(bonusPsy, messagePsy) {
    ...
    `;
    const container = div.querySelector('#zoneBarresCombat');
    ...
    if (!document.getElementById('style-barres-combat')) {
      ...
      style.textContent = `
        .barre-container { position:relative; width:320px; height:38px; border-radius:999px; background:#ddd; overflow:hidden; margin-bottom:10px; display:flex; align-items:center; box-shadow:0 0 8px rgba(0,0,0,0.2)}
        .vie-remplissage { position:absolute; height:100%; border-radius:999px; background:linear-gradient(to right,#4e9cff,#b3e6ff); transition:width .3s; z-index:1}
        .contenu-barre { position:relative; z-index:2; display:flex; align-items:center; justify-content:space-between; width:100%; padding:0 15px; color:#222}
        .nom { font-weight:bold}
        .rond-vie { width:28px;height:28px;border-radius:50%;background:#fff;border:2px solid #999;display:flex;align-items:center;justify-content:center;font-weight:bold}
        #btnHeroMoins2,#btnMonstreMoins2{padding:6px 14px;border:none;border-radius:8px;background:#333;color:#fff;cursor:pointer;margin-bottom:10px;box-shadow:2px 2px 5px rgba(0,0,0,0.2)}
        #btnHeroMoins2:hover,#btnMonstreMoins2:hover{background:#555}
      `;
    ...
    const maj = (idBarre, idRest, idIco, val, max, icoP, icoV) => {
