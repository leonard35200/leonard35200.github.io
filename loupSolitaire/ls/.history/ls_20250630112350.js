...
class NavigationManager {
  ...
  // On ne déclenche que si c'est un paragraphe de combat
  if (!p.innerHTML.includes('<br><br><strong>')) return;

  // Nettoyage d'une éventuelle fenêtre de combat précédente
  ...
  // Extraction des stats du monstre
  let vieMonstre  = 10;
  let habMonstre  = 10;
  ...
  if (isNaN(habHero)) {
    habHero = +document.getElementById('hab').value || 15;
  }

  // Bonus
  const armesPsy      = Object.values(localStorage).some(v => v && v.includes("Puissance psychique"));
  const bouclierPsy   = Object.values(localStorage).some(v => v && v.includes("Bouclier psychique"));
  let   bonusPsy      = 0;
  let   bonusArme     = 0;
  const armeMaitrisee = localStorage.getItem('arme_maitrisee');
  const dispArme      = localStorage.getItem('discipline1');
  if (dispArme && dispArme.includes("Maîtrise des armes") &&
  ...
  // Création du container de combat
  ...
  if (armesPsy) {
    ...
    if (btn2) {
      btn2.addEventListener("click", () => {
        const cd = div.querySelector("#confirmationPsy");
        console.log("[Combat] btnPsy2 clic, confirmationPsy trouvé :", cd);
        if (cd) cd.remove();
        div.querySelector("#zoneBarresCombat").style.display = "";
        afficherBarres(2, "(Psychique)");
      });
    } else {
      console.warn("[Combat] btnPsy2 introuvable");
    }

    if (btn0) {
      btn0.addEventListener("click", () => {
        const cd = div.querySelector("#confirmationPsy");
        console.log("[Combat] btnPsy0 clic, confirmationPsy trouvé :", cd);
        if (cd) cd.remove();
        div.querySelector("#zoneBarresCombat").style.display = "";
        afficherBarres(0, "");
      });
    } else {
      console.warn("[Combat] btnPsy0 introuvable");
    }
